...
[`../../BOSS-STANDARD.md`](../../BOSS-STANDARD.md), which lists **Reversible** — "High-impact
actions are gated, versioned, and recoverable" — as one of the four governance surfaces
...
That last moment — recovery reproducing the *exact* prior state, not an approximation —
is the whole point.
...
- **In-place mutation fails closed.** `_update_in_place()` and `_delete_in_place()` model
  ...
  `restore()` is refused unconditionally, every time — not "usually."
- **`restore(N)` is recovery, not rewind.** It reconstructs a prior state by appending a
  ...
  here means *deterministic*, not best-effort.
...
refused every time, not just usually. This is a real, checkable property, demonstrated
by code you can run.

**Does NOT prove / is NOT:** this is a **generic state-versioning primitive**, not the
BOSNet.io engine, kernel, or spec (patent-pending, reserved). It does not implement HITL
