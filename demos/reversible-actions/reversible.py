...
operator-destructive path writes its own audit note instead of silently erasing state.
...
class Store:
    ...
    way: by appending a new version equal to it, always moving forward through history.
    ...
    def restore(self, version: int) -> Version:
        ...
        appending a NEW version equal to `version`'s state. Deterministic: the
        returned version's state_hash equals snapshot_at(version)
