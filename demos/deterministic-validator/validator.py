...
A proposal that fails is DISCARDED, not patched: nothing here ever rewrites a failing
...
def propose(intent: str, source_id: str, amount_usd: int, model: str = "claude") -> Proposal:
    ...
    a plain constructor, because propose() is not the thing under test — validate() is.
...
class Validator:
    ...
    `rejected_log` records every discard with its reason — the failure is recorded, not
    hidden. There is deliberately no repair path: a proposal either commits verbatim or it
    ...
    def submit(self, proposal: Proposal, judgment: Optional[Judgment] = None) -> CheckResult:
        ...
        — it is accepted here purely so callers can log it alongside the decision; it is
        ...
        `rejected_log` with its reason and is otherwise discarded, not patched.
        """
