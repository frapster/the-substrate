...
Most of software's cost accrues after it ships, over its maintenance life.

The canonical framing is the **60/60 rule**: roughly 60% of a system's lifetime cost is maintenance, and
about 60% of *that* is enhancement rather than bug-fixing ([O'Reilly, *97 Things Every Project Manager Should
...
context-dependent: regulated domains trend higher, simple marketing sites lower. Treat specific
...
| Relic Wars | ~19k LOC (algorithmic engine) | ~80-90% (algorithmic, the contrast case) |
...
The per-system before/after TCO estimates live in each case study. The through-line: **LLM-First pays
off as a smaller thing to maintain for a decade.**
...
orchestrator** rather than a typist. That is a strength only if it is governed, because the published evidence
...
this says one thing clearly: **LLM output is a draft that needs something to enforce quality and
security before it ships.** The answer in these systems is a layered, enforced gate stack:
...
staying true.** The harness beneath it must not have *changed* (a third-party runtime you don't control
can shift under you between sessions, silently invalidating an assumption your governance rested on), and
you must not have *forgotten to invoke it* (a rule not loaded into context, a hook not registered, a check
not called is simply not enforced. **Omission is ungoverned**.) **Managing drift dies here first**, at
the seams of the overlay itself: the un-invoked hook, the silently-changed harness. Governance-by-invocation
is fragile precisely because it depends on remembering, every time.
...
recurring maintenance category in the commit history: a real, ongoing cost rather than a one-time fix.
...
it*. More retrieval alone does not fix this. The remedy is feeding the model the right context from a
...
Say this precisely, because it is easy to overstate. The problem is **naive vector-similarity retrieval
over an open corpus with no provenance.** Similarity returns what is *near* rather than what is *governed
and true*, and the failure modes are documented: retrieved context does not
...
The fix keeps retrieval and upgrades the *index*: structured, provenance-carrying knowledge
...
Code is what you write when you lack the other two. Governance bounds what the model may do;
...
context fed wrong. Managing it means holding **both** halves continuously, not just guarding the code.

The model is not to blame. **Governing an LLM-first build is an active,
...
**"'Governance + knowledge' is a false trichotomy."** Governance rules and knowledge models are
...
**On the 90:10 number.** It is a **design aspiration.** No study shows a
...
Relic Wars' whole game stays low): a measured north star rather than a claim of proven optimum.

## 7. Governance built to free people
...
The motivation research explains *why*: autonomy is *ownership of one's action* rather than the absence
of structure, so good constraints can raise felt autonomy rather than lower it ([Self-Determination Theory,
...
governance *starts*: NIST's AI RMF makes **GOVERN** a culture-and-people function that supports the others
...
must carry its conditions. **Governed tools free people when the framework is a paved path rather than
a fence:** product-managed, optional where it can be, safe-by-default, and built to remove friction
rather than to watch the worker. That is the bar this work holds itself to. It is not automatic, and
saying so is part of the discipline.
