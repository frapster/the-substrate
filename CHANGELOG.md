...
guarantee (and the two originals — governed knowledge and the 90:10 ratio — that previously had no
...
- **`demos/` — a suite of runnable proofs**, each stdlib-only, with a narrative runner, `unittest`
  ...
  - `demos/deterministic-validator/` — commits only what passes; a rejected proposal is discarded, not
    patched. Proves the validator decision (ADR-0003).
...
### Changed
- **`demo/` → `demos/audit-ledger/`.** The original tamper-evident ledger moved into the suite
