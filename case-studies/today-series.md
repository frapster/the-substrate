...
- **Standout risk:** a **thin test surface**: 3 test files, ~162 lines. For a platform this is the
  maintenance liability the conversion must address rather than inherit.
...
2. **Compile, don't branch.** A governed compiler turns a blueprint + retrieved evidence into a run-time
   plan: curation, sequencing, and metadata become compiled reasoning rather than enumerated code.
...
load-bearing controls rather than nice-to-haves.
